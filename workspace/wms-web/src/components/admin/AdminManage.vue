...
const name = ref('')
const sex = ref('')
const isValid = ref('') // 默认不筛选，显示全部用户
  ...
  formRef.value.validate((valid) => {
    if (valid) {
      ...
      const submitData = {
        ...
        roleId: form.value.roleId ? parseInt(form.value.roleId) : 1, // 默认普通用户
        isvalid: 'Y'
      }
...
const handleStatusChange = (row) => {
  const originalStatus = row.isValid === 'Y' ? 'N' : 'Y' // 保存原始状态用于回滚
  
  // 如果原始状态为空，则视为'Y'（正常）
  const newStatus = row.isValid || 'Y'
  
  axios.post(httpUrl+'/user/updateStatus', {
    id: row.id,
    isValid: newStatus
  }).then(res => {
    if(res.data.code == 200) {
      $message.success(res.data.data || '状态更新成功')
      // 确保数据库更新后前端状态正确
      row.isValid = newStatus
    } else {
      // 失败时还原状态
      row.isValid = originalStatus
      $message.error(res.data.data || '状态更新失败')
    }
  }).catch(error => {
    // 异常时还原状态
    row.isValid = originalStatus
...
<template>
  <div>
    <div style="margin-bottom: 10px;">
      ...
      <el-select v-model="isValid" @change="loadPost" placeholder="账号状态" style="width: 150px; margin-left: 10px;">
        <el-option
      ...
      <el-table-column prop="roleId" label="角色" width="120">
        <template #default="scope">
          <el-tag
              ...
              disable-transitions>{{scope.row.roleId=== 0 ? '系统管理员' : (scope.row.roleId=== 1 ? '管理员' : '用户')}}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="save" :loading="centerDialogVisible">确 定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
