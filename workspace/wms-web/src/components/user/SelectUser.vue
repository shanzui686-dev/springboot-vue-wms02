...
const selectCurrentChange = (val) =>{
  console.log('========== SelectUser: 选择用户 ==========')
  console.log('选中行的完整数据:', val)
  console.log('用户ID:', val?.id, '类型:', typeof val?.id)
  console.log('用户姓名:', val?.name)
  
  if(val && val.id) {
    console.log('emit触发，传递用户数据')
    emit('select', val)
  } else {
    console.error('用户数据不完整，无法emit')
  }
}
const add = () => {
